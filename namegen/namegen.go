...
func init() {
	func() {
		t := raws.NewTokenizer(strings.NewReader(languagewords))
		for {
			tok, err := t.Next()
			if err == io.EOF {
				break
	...
	for _, l := range []struct {
		...
		{languageDWARF, func(w *Word, s string) { w.Translation.Dwarf = s }},
		{languageHUMAN, func(w *Word, s string) { w.Translation.Human = s }},
		{languageGOBLIN, func(w *Word, s string) { w.Translation.Goblin = s }},
		{languageELF, func(w *Word, s string) { w.Translation.Elf = s }},
	} {
		l := l
		func() {
			t := raws.NewTokenizer(strings.NewReader(l.L))
			for {
				tok, err := t.Next()
				if err == io.EOF {
					break
